   <div class="hidden w-4/12 -mx-8 lg:block">
            ...
            <div class="flex flex-col px-6 py-4 mx-auto bg-white rounded-lg shadow-md">
                <ul class="-mx-4">
                    ...
                    <li class="flex items-center mt-6"><img
                        ...
                        <p><a href="{{ route('auteur',$user) }}" class="mx-1 font-bold text-gray-700 hover:underline">{{ $user->name }}<span
                                class="text-sm font-light text-gray-700"> - {{ $user->posts->count() }} Posts</a></span></p>
                    </li>
                    @endif
                    @endforeach
                    {{-- 
        ...
        <div class="px-8 mt-10">
            <h1 class="mb-4 text-xl font-bold text-gray-700">Dernier Post</h1>
            <div class="flex flex-col px-8 py-6 mx-auto bg-white rounded-lg shadow-md">
                <div class="flex items-center justify-center"><a href="#"
                        class="px-2 py-1 text-sm text-green-100 bg-gray-600 rounded hover:bg-gray-500">{{ $firstPost->category->name }}</a>
                </div>
                <div class="mt-4"><a href="#" class="text-lg font-medium text-gray-700 hover:underline">{{ $firstPost->title }}</a></div>
                <div class="flex items-center justify-between mt-4">
                    <div class="flex items-center"><img
                            src="https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=crop&amp;w=731&amp;q=80"
                            alt="avatar" class="object-cover w-8 h-8 rounded-full"><a href="#"
                            class="mx-3 text-sm text-gray-700 hover:underline">{{ $firstPost->user->name }}</a></div><span
                        class="text-sm font-light text-gray-600">{{  $firstPost->created_at->format('d/m/y : h:i:s')
                        }}</span>
                </div>
            </div>
        </div>
